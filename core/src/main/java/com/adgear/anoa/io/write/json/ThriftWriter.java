...
class ThriftWriter<F extends TFieldIdEnum, T extends TBase<T,F>> extends JsonWriter<T> {

  final private LinkedHashMap<F,JsonWriter<Object>> fieldMap;
  ...
  ThriftWriter(Class<T> thriftClass) {
    ...
    FieldMetaData.getStructMetaDataMap(thriftClass).entrySet().stream()
        ...
        .forEach(e -> fieldMap.put((F) e.getKey(),
                                   (JsonWriter<Object>) createWriter(e.getValue().valueMetaData)));
  }

  @Override
  public void write(T t, JsonGenerator jsonGenerator) throws IOException {
    jsonGenerator.writeStartObject();
    for (Map.Entry<F,JsonWriter<Object>> entry : fieldMap.entrySet()) {
      F f = entry.getKey();
      if (t.isSet(f)) {
  ...
  static protected JsonWriter<?> createWriter(FieldValueMetaData metaData) {
