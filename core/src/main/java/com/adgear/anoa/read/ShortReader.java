...
class ShortReader extends AbstractReader<Short> {
  ...
  @Override
  protected Short readStrict(JsonParser jacksonParser) throws AnoaTypeException, IOException {
    switch (jacksonParser.getCurrentToken()) {
      case VALUE_NUMBER_INT:
        final int intValue = jacksonParser.getIntValue();
        if (intValue > Short.MAX_VALUE || intValue < Short.MIN_VALUE) {
          throw new AnoaTypeException(jacksonParser.getText() + " is out of bounds for I16.");
        }
