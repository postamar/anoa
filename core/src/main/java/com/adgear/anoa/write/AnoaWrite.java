...
public class AnoaWrite {
  ...
  static public <T, JG extends JsonGenerator> @NonNull AnoaFunction<T, JG> anoaFn(
    ...
    return AnoaFunction.pokemon(fnWrap(biCo(avroOrThriftClass), jsonGeneratorSupplier),
  ...
  static public <JG extends JsonGenerator> @NonNull AnoaFunction<GenericRecord, JG> anoaFn(
      ...
      @NonNull Supplier<@NonNull JG> jsonGeneratorSupplier,
      Class context) {
    return AnoaFunction.pokemon(fnWrap(biCo(avroSchema), jsonGeneratorSupplier),
                                AvroWriter.class);
  }
  ...
  static public <T> @NonNull BiConsumer<T, JsonGenerator> biCo(
      @NonNull Class<T> avroOrThriftClass) {
    return (BiConsumer<T, JsonGenerator>) objectFactory.get(avroOrThriftClass);
  }
  ...
  static final CachedFactory<Schema, BiConsumer<GenericRecord, JsonGenerator>> genericFactory =
      new CachedFactory<>(AvroWriter::new);
  static final CachedFactory<Class, BiConsumer<?, JsonGenerator>> objectFactory =
      new CachedFactory<>(AnoaWrite::createWriter);

  @SuppressWarnings("unchecked")
  static private BiConsumer<?, JsonGenerator> createWriter(Class avroOrThriftClass) {
    if (TBase.class.isAssignableFrom(avroOrThriftClass)) {
      return new ThriftWriter<>(avroOrThriftClass);
    } else if (SpecificRecord.class.isAssignableFrom(avroOrThriftClass)) {
      return new AvroWriter<>(avroOrThriftClass);
    }
